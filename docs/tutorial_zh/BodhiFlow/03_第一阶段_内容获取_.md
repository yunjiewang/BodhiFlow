...
这是通过 Python 的 `ProcessPoolExecutor` 实现的，让我们看看 `core/nodes.py` 中被大大简化的 `exec` 方法：
...
若在界面中选择 **ZAI GLM-ASR-2512** 作为语音转文字模型，会有两点自动行为：

1. **音频格式**：该接口仅接受 .wav 或 .mp3。为节省空间并避免重复转码，从本地视频或 Teams 录制提取音频时，会**直接输出 MP3**，分块也是 MP3，无需在 STT 前再转换。
2. **并发上限**：ZAI 限制同时最多 5 个 ASR 请求。因此当选用该模型时，第一阶段的**并行进程数会自动限制在 5 以内**（由配置中的 `max_concurrency` 控制），避免超出接口限制。

详见 `config/models_config.json` 中 ASR 模型配置的 `max_chunk_duration_seconds`（如 30 秒）和 `max_concurrency`（如 5）。
